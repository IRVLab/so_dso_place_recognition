classdef ProgressConsoleMatch < handle
    ...
    methods
        function obj = ProgressConsoleMatch(config, diff_m, varargin)
            % Save the config
            obj.config = config;
            ...
            % Attach predefined difference matrix
            obj.instance.results.diff_matrix.base = diff_m;
        end
        ...
        function run(obj)
            obj.instance.contrastEnhancement();
            obj.instance.matching();
            obj.instance.thresholding();
            matchingSC = obj.instance.results.matching.all.min_scores;
            save(strcat(obj.config.results.path, '/matchingSC.mat'), 'matchingSC');
