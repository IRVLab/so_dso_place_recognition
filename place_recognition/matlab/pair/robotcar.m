...
pause(0.5);
    ...
    function[AUC, top_recall] =
        run_test(run1, run2, run_count, type) start_idx = 0;
end_idx = 0;
loop_diff = 25;

incoming1_id = load(
    strcat(strcat('../../results/RobotCar/', run1), '/incoming_id_file.txt'));
incoming2_id = load(
    strcat(strcat('../../results/RobotCar/', run2), '/incoming_id_file.txt'));
gt1_full = load(strcat(strcat('../../results/RobotCar/', run1), '/gps.txt'));
gt2_full = load(strcat(strcat('../../results/RobotCar/', run2), '/gps.txt'));
incoming1_id = incoming1_id(start_idx + 1
                            : size(incoming1_id, 1) - end_idx,
                            :) +
               1;
incoming2_id = incoming2_id(start_idx + 1
                            : size(incoming2_id, 1) - end_idx,
                            :) +
               1;
...
if (~strcmp(type, 'BoW'))
  hist1 = load(strcat(
      strcat(strcat(strcat('../../results/RobotCar/', run1), '/history'), type),
      '.txt'));
hist2 = load(strcat(
    strcat(strcat(strcat('../../results/RobotCar/', run2), '/history'), type),
    '.txt'));
end

        tic %
    get difference matrix switch type case 'M2DP' diff_m =
    processM2DP(hist1, hist2, start_idx, end_idx);
    case 'SC'
    ...
    diff_m = 1 - diff_m;
        ...
        [AUC, top_recall] =
            getAUCandPlot(diff_m, gt1, gt2, loop_diff, run_count);
    end
