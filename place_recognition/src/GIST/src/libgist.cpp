...
static void fftw_lock(void) { fftw_mutex.lock(); }

static void fftw_unlock(void) { fftw_mutex.unlock(); }
...
static image_t *image_add_padding(image_t *src, int padding) {
  ...
  for (j = 0; j < src->height; j++) {
    for (i = 0; i < src->width; i++) {
      img->data[(j + padding) * img->stride + i + padding] =
          src->data[j * src->stride + i];
    }
  }

  for (j = 0; j < padding; j++) {
    for (i = 0; i < src->width; i++) {
      img->data[j * img->stride + i + padding] =
          src->data[(padding - j - 1) * src->stride + i];
  ...
  for (j = 0; j < img->height; j++) {
    for (i = 0; i < padding; i++) {
      img->data[j * img->stride + i] =
          img->data[j * img->stride + padding + padding - i - 1];
      img->data[j * img->stride + i + padding + src->width] =
          img->data[j * img->stride + img->width - padding - i - 1];
    }
  }
...
static color_image_t *color_image_add_padding(color_image_t *src, int padding) {
  ...
  color_image_t *img =
      color_image_new(src->width + 2 * padding, src->height + 2 * padding);

  for (j = 0; j < src->height; j++) {
    for (i = 0; i < src->width; i++) {
      img->c1[(j + padding) * img->width + i + padding] =
          src->c1[j * src->width + i];
      img->c2[(j + padding) * img->width + i + padding] =
          src->c2[j * src->width + i];
      img->c3[(j + padding) * img->width + i + padding] =
          src->c3[j * src->width + i];
    }
  }

  for (j = 0; j < padding; j++) {
    for (i = 0; i < src->width; i++) {
      img->c1[j * img->width + i + padding] =
          src->c1[(padding - j - 1) * src->width + i];
      img->c2[j * img->width + i + padding] =
          src->c2[(padding - j - 1) * src->width + i];
      img->c3[j * img->width + i + padding] =
          src->c3[(padding - j - 1) * src->width + i];
  ...
  for (j = 0; j < img->height; j++) {
    for (i = 0; i < padding; i++) {
      img->c1[j * img->width + i] =
          img->c1[j * img->width + padding + padding - i - 1];
      img->c2[j * img->width + i] =
          img->c2[j * img->width + padding + padding - i - 1];
      img->c3[j * img->width + i] =
          img->c3[j * img->width + padding + padding - i - 1];

      img->c1[j * img->width + i + padding + src->width] =
          img->c1[j * img->width + img->width - padding - i - 1];
      img->c2[j * img->width + i + padding + src->width] =
          img->c2[j * img->width + img->width - padding - i - 1];
      img->c3[j * img->width + i + padding + src->width] =
          img->c3[j * img->width + img->width - padding - i - 1];
    }
  }
  ...
  for (j = 0; j < dest->height; j++) {
    for (i = 0; i < dest->width; i++) {
      dest->data[j * dest->stride + i] =
          src->data[(j + padding) * src->stride + i + padding];
    }
  }
}
...
static void color_image_rem_padding(color_image_t *dest, color_image_t *src,
  ...
  for (j = 0; j < dest->height; j++) {
    for (i = 0; i < dest->width; i++) {
      dest->c1[j * dest->width + i] =
          src->c1[(j + padding) * src->width + i + padding];
      dest->c2[j * dest->width + i] =
          src->c2[(j + padding) * src->width + i + padding];
      dest->c3[j * dest->width + i] =
          src->c3[(j + padding) * src->width + i + padding];
    }
  }
}
...
static image_list_t *create_gabor(int nscales, const int * or, int width,
                                  int height) {
  int i, j, fn;
  ...
  for (fn = 0; fn < nfilters; fn++) {
    ...
    for (j = 0; j < height; j++) {
      for (i = 0; i < width; i++) {
        ...
        G0->data[j * G0->stride + i] =
            exp(-10.0f * param[fn][0] * (*fr_ptr / height / param[fn][1] - 1) *
                    (*fr_ptr / width / param[fn][1] - 1) -
                2.0f * param[fn][2] * M_PI * tmp * tmp);
        fr_ptr++;
      }
    }
...
/*static*/ void prefilt(image_t *src, int fc) {
  ...
  for (j = 0; j < src->height; j++) {
    for (i = 0; i < src->width; i++) {
      src->data[j * src->stride + i] =
          log(src->data[j * src->stride + i] + 1.0f);
    }
  }
  ...
  fftwf_complex *in1 =
      (fftwf_complex *)fftwf_malloc(width * height * sizeof(fftwf_complex));
  fftwf_complex *in2 =
      (fftwf_complex *)fftwf_malloc(width * height * sizeof(fftwf_complex));
  fftwf_complex *out =
      (fftwf_complex *)fftwf_malloc(width * height * sizeof(fftwf_complex));
  ...
  /* FFT */
  fftwf_plan fft1 =
      fftwf_plan_dft_2d(width, height, in1, out, FFTW_FORWARD, FFTW_ESTIMATE);
  fftw_unlock();
  fftwf_execute(fft1);
  fftw_lock();
  ...
  /* IFFT */
  fftwf_plan ifft1 =
      fftwf_plan_dft_2d(width, height, out, in2, FFTW_BACKWARD, FFTW_ESTIMATE);
  fftw_unlock();
  fftwf_execute(ifft1);
  fftw_lock();
  ...
  for (j = 0; j < height; j++) {
    for (i = 0; i < width; i++) {
      img_pad->data[j * stride + i] =
          img_pad->data[j * stride + i] /
          (0.2f + sqrt(sqrt(in2[j * width + i][0] * in2[j * width + i][0] +
                            in2[j * width + i][1] * in2[j * width + i][1]) /
                       (width * height)));
    }
  }
...
static void color_prefilt(color_image_t *src, int fc) {
  ...
  /* FFT */
  fftwf_plan fft11 =
      fftwf_plan_dft_2d(width, height, ina1, out1, FFTW_FORWARD, FFTW_ESTIMATE);
  fftwf_plan fft12 =
      fftwf_plan_dft_2d(width, height, ina2, out2, FFTW_FORWARD, FFTW_ESTIMATE);
  fftwf_plan fft13 =
      fftwf_plan_dft_2d(width, height, ina3, out3, FFTW_FORWARD, FFTW_ESTIMATE);
  fftw_unlock();
  fftwf_execute(fft11);
  fftwf_execute(fft12);
  fftwf_execute(fft13);
  ...
  for (j = 0; j < height; j++) {
    for (i = 0; i < width; i++) {
      ...
      float mean = (img_pad->c1[j * width + i] + img_pad->c2[j * width + i] +
                    img_pad->c3[j * width + i]) /
                   3.0f;
  ...
  /* IFFT */
  fftwf_plan ifft2 = fftwf_plan_dft_2d(width, height, out1, inb1, FFTW_BACKWARD,
                                       FFTW_ESTIMATE);
  fftw_unlock();
  fftwf_execute(ifft2);
  fftw_lock();
  ...
  for (j = 0; j < height; j++) {
    for (i = 0; i < width; i++) {
      float val = sqrt(sqrt(inb1[j * width + i][0] * inb1[j * width + i][0] +
                            inb1[j * width + i][1] * inb1[j * width + i][1]) /
                       (width * height));
...
/*static*/ float *gist_gabor(image_t *src, const int w, image_list_t *G) {
  ...
  /* FFT */
  fftwf_plan fft =
      fftwf_plan_dft_2d(width, height, in1, out1, FFTW_FORWARD, FFTW_ESTIMATE);
  fftwf_plan ifft =
      fftwf_plan_dft_2d(width, height, out2, in2, FFTW_BACKWARD, FFTW_ESTIMATE);
  ...
  for (k = 0; k < G->size; k++) {
    for (j = 0; j < height; j++) {
      for (i = 0; i < width; i++) {
        out2[j * width + i][0] =
            out1[j * width + i][0] * G->data[k]->data[j * stride + i];
        out2[j * width + i][1] =
            out1[j * width + i][1] * G->data[k]->data[j * stride + i];
      }
    }
    ...
    for (j = 0; j < height; j++) {
      for (i = 0; i < width; i++) {
        src->data[j * stride + i] =
            sqrt(in2[j * width + i][0] * in2[j * width + i][0] +
                 in2[j * width + i][1] * in2[j * width + i][1]) /
            (width * height);
      }
    }
...
static float *color_gist_gabor(color_image_t *src, const int w,
                               image_list_t *G) {
  fftw_lock();
  ...
  /* FFT */
  fftwf_plan fft1 = fftwf_plan_dft_2d(width, height, ina1, outa1, FFTW_FORWARD,
                                      FFTW_ESTIMATE);
  fftwf_plan fft2 = fftwf_plan_dft_2d(width, height, ina2, outa2, FFTW_FORWARD,
                                      FFTW_ESTIMATE);
  fftwf_plan fft3 = fftwf_plan_dft_2d(width, height, ina3, outa3, FFTW_FORWARD,
                                      FFTW_ESTIMATE);
  ...
  for (k = 0; k < G->size; k++) {
    ...
    for (j = 0; j < height; j++) {
      for (i = 0; i < width; i++) {
        src->c1[j * width + i] =
            sqrt(inb1[j * width + i][0] * inb1[j * width + i][0] +
                 inb1[j * width + i][1] * inb1[j * width + i][1]) /
            (width * height);
        src->c2[j * width + i] =
            sqrt(inb2[j * width + i][0] * inb2[j * width + i][0] +
                 inb2[j * width + i][1] * inb2[j * width + i][1]) /
            (width * height);
        src->c3[j * width + i] =
            sqrt(inb3[j * width + i][0] * inb3[j * width + i][0] +
                 inb3[j * width + i][1] * inb3[j * width + i][1]) /
            (width * height);
      }
    }
...
float *bw_gist_scaletab(image_t *src, int w, int n_scale,
  ...
  image_list_t *G =
      create_gabor(n_scale, n_orientation, img->width, img->height);
