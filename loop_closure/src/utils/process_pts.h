...
void getPtsSphere(const IDPose *cur_pose,
  ...
  // pts_sphere); filterPointsPolar(pts_sphere_raw, loopRange, {1,1,0.1},
  // pts_sphere);
  filterPointsPolar(pts_sphere_raw, loopRange, {voxelAngle, voxelAngle, 1},
                    pts_sphere, false);
...
void process_pts(std::string &poses_history_file, std::string &pts_history_file,
                 std::string &incoming_id_file, double loopRange,
                 double voxelAngle,
  ...
  for (auto &cur_pose : poses_history) {
    ...
    // get sphere pts
    pts_sphere.clear();
    getPtsSphere(cur_pose, nearby_pts, pts_sphere, loopRange, voxelAngle);
